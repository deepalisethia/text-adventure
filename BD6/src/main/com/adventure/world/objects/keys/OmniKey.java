...
public class OmniKey implements Tangible {
    ...
    public void useItem(Tangible door) {
        if (door instanceof OmniDoor) {
            ...
            boolean[] newPins = ((OmniDoor) door).getPins();
            for (int i = 0; i < pins.length; i++) {
                pins[i] = newPins[i];
            }
            System.out.println("The key is configured.");
        } else {
            System.out.println("This key can only be used with OmniDoors and nothing more.");
        }
    }
